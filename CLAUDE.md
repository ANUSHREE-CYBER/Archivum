...
- **Crossover is films-only, and comics never cross.** `matchesTypeTab()` has exactly one call site (the `sections` memo) and three exclusions, all deliberate:
  - Movies matches `type === 'movie' || format === 'movie'` — so an anime or kdrama *film* does cross, which is the whole reason the `format` column exists.
  - TV Show excludes `kdrama` **and `anime`**. Both get `format: 'series'` automatically on import (TMDB tv → `series`, AniList TV/OVA/ONA → `series`), so without the guard every anime series rendered a second time under TV Show purely as an artefact of how it was imported. The asymmetry with Movies is intentional: a series already has its own section, so crossing it only duplicates it.
  - **Both** branches exclude `manga`/`manhwa` outright, ignoring `format` entirely. A comic is never a film or a show; the guard is a hard floor against rows saved before `ManualEntryModal` stopped offering Movie/Series for those types.
...
- `format` column: nullable `'movie' | 'series' | 'comic'` for cross-category classification (e.g. anime films). `ManualEntryModal` only offers `—/Movie/Series`, and only for the four watch types — the Format selector is hidden for `book` (as before) and now for `manga`/`manhwa` too, which get `'comic'` written on save to match what `MediaSearch` stores for imported manga. `'comic'` is therefore never user-selectable.
...
- **`Dropdown`'s menu must keep `onMouseDown={e => e.preventDefault()}`.** Without it the component is unusable with a mouse — see the entry under Known issues. Do not "clean it up".
...
- **RESOLVED (2026-08-18) — `Dropdown` options were unclickable with a mouse.** The menu opened, but clicking any option did nothing: the value never changed and the menu just blinked shut. Keyboard selection worked fine, which is why it survived so long. Cause was **not** the outside-click listener (`rootRef` wraps the menu, so `contains()` correctly returns true for options). It was the `onBlur` on the root div. The options are `<div role="option">` with no `tabIndex` and nothing in the menu is focusable, so a real `mousedown` on one runs its default action and moves focus off the trigger to `<body>`. That fires `focusout` with `relatedTarget: null`; the handler reads `contains(null) === false` as "focus left the dropdown" and closes the menu **during `mousedown`, before `mouseup`** — so the option is unmounted and the browser never dispatches `click` to it. Fixed with `onMouseDown={e => e.preventDefault()}` on the menu container, which suppresses the focus shift and enforces the invariant the component already documents ("focus stays on the trigger the whole time"). Two lessons: (1) the event trace is `pointerdown` → `mousedown` → `focusout` → `mouseup` with **no `click`** — that shape means the target was removed mid-gesture, not that something is intercepting; (2) **`el.click()` cannot reproduce this.** A synthetic dispatch involves no `mousedown` and no focus change, so automated checks using it passed the whole time. Verify pointer behaviour with real input (CDP `Input.dispatchMouseEvent`), and sanity-check the input pipe first by clicking an injected throwaway element — headless Chrome silently drops synthesized input in some states, which looks identical to the bug.
- **RESOLVED (2026-08-18) — anime series cross-listed into TV Show.** Any anime with `format: 'series'` (i.e. most of them, set automatically at import) rendered in both Anime and TV Show. Kdrama already had an exclusion; anime needed the same one, plus `manga`/`manhwa` are now excluded from *both* crossover branches so a stray `format` on a comic row can't leak it into Movies or TV Show. See the crossover bullet under "Vault page → Sections" for the full current rule.
